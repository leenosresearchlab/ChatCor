...
(function() {
    ...
    $('#txtProjectName').on('show.bs.tooltip', function (event) {
        $(this).data("istooltip", true);
    });

    $('#txtProjectName').on('hide.bs.tooltip', function (event) {
    ...
    $("#btnNewProject").on('click', function (evt) {
        evt.preventDefault();
        if (!$('#txtProjectName').data("istooltip")) {
            fnNewProject(evt);
        } else {
...
var fnNewProject = function (obj) {
    var projectName = $("#txtProjectName").val().trim(),
        pattern = /^[a-zA-Z]+$/gi,
        param = $('#frmProject').serializeObject();
